...
export function LeadForm({ listingId, zakelijk = false }: { listingId: string; zakelijk?: boolean }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-2">
      ...
      <input type="tel" className="field" placeholder={t("lead.telefoon")} value={form.telefoon} onChange={(e) => setForm({ ...form, telefoon: e.target.value })} />
      ...
      {fout && <div className="rounded-xl bg-[#FBEEE4] border border-[#F0D6C1] text-oranje-dk p-2.5 text-sm">{t("lead.fout")}</div>}
      <button className="btn w-full" disabled={loading}>{loading ? t("form.versturen") : t("listing.send")}</button>
    </form>
  );
}
